...
|---|---|
| Photos | 400 |
| Plan tier | premium (max dimension 4096px) |
| Distinct source images | 16 (cycled) |
| Batch size (continuation) | 20 photos |
| **Wall-clock (encode + zip + finalize)** | **1202.4 s** |
| **Peak RSS** | **2247.95 MiB** |
| **Archive size** | **1886.1 MiB** |
| Per-photo wall-clock | 3005.9 ms |
| Per-photo JPEG encode (sharp) | 2992.2 ms |
| Avg WebP source | 4232 KiB |
| Avg exported JPEG | 4828 KiB |
...
- **JPEG is derived at export time** from the stored WebP variant (never AVIF).
  ...
  (2992.2 ms); the ZIP writer (STORE, per-file CRC-32) is
  negligible next to it.
- **Peak RSS (2247.95 MiB)** is the conservative figure: this bench
  ...
  than one batch of JPEGs (~94 MiB) at a time.
