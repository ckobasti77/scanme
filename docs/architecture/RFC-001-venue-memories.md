...
Remaining block types: `eventDateTime` (start/end, venue name/address, Google-Calendar link + generated `.ics` toggles — the add-to-calendar constraint), `map` (address or lat/lng, zoom, pin label, static vs embed), `profileCards` (formerly `performerCards`), `priceList` (formerly `menu`; sections → items, currency), `reservation` (field config, capacity, deadline; submissions in `venueReservations`), `share` (channels, prefilled message), `pastEvents` (auto-sourced from archived events), plus `richText` and `spacer` utilities. Every block type's `defaults` and `clamp` live in pure `lib/venue-blocks.ts` so `convex/venue.ts` normalizes on write exactly the way `normalizeDesignForPreset` does for Links today.

**Naming note (TASK-10, 2026-08-25): ScanMe Menu is a planned separate product, and Venue must not absorb it.** A dedicated menu/price-list product for services and hospitality is on the roadmap under the name **ScanMe Menu**. Venue's block formerly called `menu` was renamed **`priceList`** (while `venueEventConfigs` was still empty, so no migration) to keep the word "Menu" — in the UI, in support conversations, and in this codebase — free for that product. Venue's `priceList` block is a light convenience for listing a few priced items on a campaign page; it is **not** a replacement for ScanMe Menu, and nobody should grow Venue into a menu product. In the same pass `performerCards` became **`profileCards`**: the structure (image, title, subtitle) is a stylist, a trainer, or a workshop leader as much as a DJ, and Venue's vocabulary stays niche-neutral.
...
| **Retention** | `convex/crons.ts` daily `retentionSweep` (internal mutation) | Per space: `cutoff = now − retentionDays`; range-delete via `by_spaceId_and_createdAt` in batches with `ctx.scheduler.runAfter(0, …)` continuations (the guidelines' bulk pattern); marks `deleted/retention`. **Exception — the archive pin outlives retention (TASK-23):** `retentionSweepSpace` skips a photo that still has a live `eventArchiveItems` row (read via `by_mediaAssetId` on the photo's committed asset). A photo the host pinned onto the venue's public page is curated content, not just part of the night, so the retention clock — a storage policy — must not silently empty a `pastEvents` block. This is *only* a retention carve-out: an **explicit** deletion (guest wipe, guest/host/admin per-photo delete, space wipe) still tombstones the photo and `purgePhotoBytes` still deletes its pins, uniformly (§2.10). A second `purgeSweep` cron walks `by_status_and_updatedAt` tombstones, deletes blobs through the `convex/lib/storage.ts` wrapper (`ctx.storage.delete`), then deletes docs; also purges stale `reserved`/`processing` rows and orphan originals. | Entirely server-scheduled; no client involvement. |
...
1. **Venue plan tiers.** ~~Unspecified~~ — **RESOLVED (TASK-43, 2026-09-01).** Basic carries the core blocks (countdown, eventDateTime, programTimeline, map, richText, share, spacer) and at most **one** active (scheduled/live) event; Premium carries **all twelve** blocks, unlimited scheduled events, and event analytics. The catalog lives in `convex/lib/plans.ts` (`PLAN_LIMITS.scanme_venue`), and `ACCOUNT_PLAN_TIER` maps account premium/enterprise → the venue premium tier (RFC-002 §2.2.3). Enforcement: allow-list at saveDraft/publishDraft **and at public read time** (a lapsed plan degrades the published page to the core set); the active-event ceiling in the same transaction as `scheduleEvent`; the analytics read gated in `venueAnalytics.eventMetrics`. Archive limits stay as shipped (`ARCHIVE_MAX_ITEMS`, tier-independent).
